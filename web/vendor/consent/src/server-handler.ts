...
// ─── Validation (lightweight zod-style without runtime dep) ──────────────────
...
function isCategoriesShape(v: unknown): v is ConsentRecord['categories'] {
...
function decodeLegacyClientPayload(consentB64: string): ConsentRecord | null {
  try {
 ...
 */
export function parseConsentBody(body: unknown): ConsentPostPayload {
  ...
  if (typeof b.consent === 'string') {
    ...
    if (!record) {
      throw new ConsentValidationError(
        'BAD_REQUEST',
        'consent field is not a valid base64 ConsentRecord'
      )
    }
    return {
  ...
  if (!isCategoriesShape(b.categories)) {
    throw new ConsentValidationError(
      'BAD_REQUEST',
      'categories must be {analytics, marketing, functional} booleans'
    )
  }
  const policyVersion =
 ...
 */
export function makeFingerprintHash(userAgent: string | null, ip: string | null): string | null {
  const ua = userAgent ?? ''
  const ipPart = ip ?? ''
  if (!ua && !ipPart) return null
 ...
 */
export async function resolveLatestCookiePolicyVersion(
  hasura: HasuraAdminConfig,
  domain: string
): Promise<LgPolicyVersionRow | null> {
  ...
  const data = (await res.json()) as {
    data?: { lg_policy_version?: LgPolicyVersionRow[] }
    errors?: unknown
  }
...
export async function insertConsentRecords(
  hasura: HasuraAdminConfig,
  payload: ConsentPostPayload,
  context: ConsentWriteContext,
  options: { action?: 'grant' | 'withdraw' | 'change' } = {}
  ...
  const rows = (Object.keys(effective) as Array<keyof typeof effective>).map((cat) => ({
  ...
  if (!res.ok) return { error: `hasura_http_${res.status}` }
  const data = (await res.json()) as {
...
export async function fetchLatestConsent(
  hasura: HasuraAdminConfig,
  context: ConsentWriteContext
): Promise<{ categories: ConsentRecord['categories']; policyVersion: string } | null> {
  const policy = await resolveLatestCookiePolicyVersion(hasura, context.domain)
  ...
  if (!res.ok) return null
  const data = (await res.json()) as {
    data?: { lg_consent_record?: Array<{ consent_type: string; granted: boolean }> }
  }
...
export async function handleConsentRequest(
  input: ConsentHandlerInput
): Promise<ConsentHandlerResult> {
  ...
  if (input.method === 'POST') {
    let payload: ConsentPostPayload
    try {
  ...
  if (input.method === 'GET') {
    ...
    if (!current) return { status: 200, body: { ok: true, record: null } }
    return {
      status: 200,
      body: {
        ok: true,
  ...
  if (input.method === 'DELETE') {
    const withdrawPayload: ConsentPostPayload = {
