...
async function searchMeilisearch(
  ...
  const params = {
    ...
    // XSS-safe highlight tags — Meilisearch escapes hit content, only these
    // exact <mark> tags are injected around matched terms.
...
export const GET: APIRoute = async ({ request }) => {
  ...
  if (!q.trim()) {
    return jsonResponse({ groups: [], total: 0, source: 'empty' })
  }
  ...
  return jsonResponse({ ...(typeParam ? filtered : results), source: 'fallback' })
}
