/**
 * SeerahExplorerIsland.tsx — Astro island port of components/seerah/seerah-explorer.tsx.
 *
 * Mechanical Next→Astro swaps only (logic preserved verbatim):
 *   - next/dynamic(ssr:false) → React.lazy + Suspense, gated behind a browser-only
 *     mount flag so the Leaflet map (which touches window/document) never runs in SSR.
 *   - next/link <Link href> → plain <a href> (same destinations).
 * REF: ports components/seerah/seerah-explorer.tsx · D-P2-STACK-CANON
 */
import React, { Suspense, lazy, useState, useEffect, useRef, useCallback, startTransition } from 'react'
import type { SeerahMapEvent } from './seerah-map'

const LazySeerahMap = lazy(() =>
...
/** Browser-only wrapper: Leaflet needs window/document, so render nothing until mounted. */
function SeerahMap(props: React.ComponentProps<typeof LazySeerahMap>) {
  const [mounted, setMounted] = useState(false)
  useEffect(() => setMounted(true), [])
  if (!mounted) return MAP_LOADING
  return (
