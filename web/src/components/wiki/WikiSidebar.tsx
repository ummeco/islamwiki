/**
 * WikiSidebar.tsx — Astro island port of components/wiki/sidebar.tsx
 *
 * PURPOSE: Left navigation for the wiki/article reading layout.
 * CONVERSION: next/link → <a>; usePathname() → window.location.pathname read on mount.
 *   Active-link highlight is computed client-side after hydration (same UX as Next).
 * CONSTRAINTS: No next/* imports. Pure presentational island; no data fetching.
 * REF: ports app components/wiki/sidebar.tsx (articles-wiki migration group)
 */
...
export function WikiSidebar() {
  ...
  return (
    <aside className="w-[220px] shrink-0 space-y-4 py-4">
      <SidebarSection title="Content" links={sections} pathname={pathname} />
      <SidebarSection title="Community" links={community} pathname={pathname} />
