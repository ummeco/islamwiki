/**
 * RashidunExplorerIsland.tsx — Astro island port of components/history/rashidun-explorer.tsx.
 *
 * Mechanical Next→Astro swap only (logic preserved verbatim):
 *   - next/dynamic(ssr:false) → React.lazy + Suspense gated behind a browser-only mount
 *     flag, so the Leaflet map never executes during SSR.
 * REF: ports components/history/rashidun-explorer.tsx · D-P2-STACK-CANON
 */
...
function RashidunMap(props: React.ComponentProps<typeof LazyRashidunMap>) {
  const [mounted, setMounted] = useState(false)
  useEffect(() => setMounted(true), [])
  if (!mounted) return MAP_LOADING
  return (
    <Suspense fallback={MAP_LOADING}>
      <LazyRashidunMap {...props} />
    </Suspense>
  )
}
