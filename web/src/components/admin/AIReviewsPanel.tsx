/**
 * AIReviewsPanel.tsx — Astro island port of app/admin/ai-reviews/page.tsx
 *
 * PURPOSE: Manual theological-compliance review trigger (Moderator 3+). Posts content to
 *   /api/ai/review and renders the verdict / confidence / issues. Hydrated via client:load.
 *
 * CONVERSION (Next → Astro):
 *   - 'use client' page → island; the fetch contract is unchanged (POST /api/ai/review).
 * SECURITY: the Anthropic key + the trustLevel>=3 gate + rate-limit all live in the
 *   /api/ai/review server route — none of that reaches this client island. The page-level
 *   admin guard (trustLevel>=4) is enforced by middleware before this island ever loads.
 * REF: ports app/admin/ai-reviews/page.tsx (auth-interactive migration group)
 */
'use client'
  ...
  return (
    <div className="section-container py-12">
      ...
      <div className="mb-8 rounded-xl border border-iw-border bg-iw-surface p-5">
        <h2 className="mb-4 text-sm font-semibold uppercase tracking-wider text-iw-accent">Review Pipeline</h2>
      ...
      <div className="rounded-xl border border-iw-border bg-iw-surface p-6">
        ...
        <div className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className="mb-1 block text-xs font-medium text-iw-text-secondary">Content Title</label>
              <input
                type="text"
                value={title}
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-iw-text-secondary">Content Type</label>
              <select
                value={contentType}
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-iw-text-secondary">Content to Review</label>
            <textarea
              value={content}
          ...
          <div className="mt-6 space-y-4">
            <div className={`rounded-xl border px-5 py-4 ${VERDICT_STYLES[result.verdict] ?? ''}`}>
              <div className="flex items-center justify-between">
                <span className="text-lg font-bold uppercase tracking-wide">{result.verdict}</span>
                <span className="text-sm opacity-80">{Math.round(result.confidence * 100)}% confidence</span>
              </div>
            ...
            {result.issues.length > 0 && (
              <div className="rounded-lg border border-iw-border bg-iw-bg/50 px-4 py-4">
                <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-iw-text-muted">Issues Found</h3>
            ...
            {(result.provider || result.model) && (
              <p className="text-xs text-iw-text-muted">Reviewed by {result.provider} · {result.model}</p>
            )}
          </div>
        )}
      </div>
