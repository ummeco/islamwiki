/**
 * AccountForm.tsx — Astro island port of app/account/account-form.tsx
 *
 * PURPOSE: Unified auth card — magic-link (default), password login, register,
 *   forgot-password, plus Google OAuth and confirmation states. Hydrated via
 *   client:load on /account.
 *
 * CONVERSION (Next → Astro):
 *   - Server actions (login / registerEmail / requestMagicLink / requestPasswordReset)
 *     → fetch() POST to the existing Astro API routes:
 *       /api/auth/signin · /api/auth/signup · /api/auth/magic-link-request · /api/auth/password-reset
 *     Each route sets httpOnly iw_at/iw_rt cookies server-side and returns { success, redirect } /
 *     { error } / { sent } — the auth secret + Turnstile verification stay server-only.
 *   - next/image → <img loading="lazy">; next/link → <a>.
 *   - Successful login/register: route via window.location.assign(redirect) (server already
 *     set the session cookie). Open-redirect guard is enforced server-side.
 * SECURITY: no admin/auth secret reaches this client island; credentials are POSTed as
 *   FormData to the cookie-setting API route exactly as the Next server actions did.
 * REF: ports app/account/account-form.tsx (auth-interactive migration group)
 */
'use client'
...
): Promise<{ error?: string; sent?: boolean; success?: boolean; redirect?: string }> {
  try {
    const res = await fetch(url, { method: 'POST', body: new FormData(form) })
    return (await res.json()) as { error?: string; sent?: boolean; success?: boolean; redirect?: string }
  } catch {
...
export function AccountForm() {
  ...
  function handleMagicLink(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await postForm('/api/auth/magic-link-request', formEl)
  ...
  function handlePassword(e: React.FormEvent<HTMLFormElement>) {
    ...
    setError('')
    startTransition(async () => {
      const result = await postForm('/api/auth/signin', formEl)
      if (result?.error) setError(result.error)
      else if (result?.success) window.location.assign(result.redirect ?? '/')
    })
  }
  ...
  function handleRegister(e: React.FormEvent<HTMLFormElement>) {
    ...
    setError('')
    startTransition(async () => {
      const result = await postForm('/api/auth/signup', formEl)
      if (result?.error) setError(result.error)
      else if (result?.success) window.location.assign(result.redirect ?? '/')
    })
  }
  ...
  function handleForgotPassword(e: React.FormEvent<HTMLFormElement>) {
    ...
    setError('')
    startTransition(async () => {
      const result = await postForm('/api/auth/password-reset', formEl)
      if (result?.error) setError(result.error)
      ...
      <div className="w-full max-w-md">
        <div className="rounded-2xl border border-iw-border bg-iw-surface p-8 shadow-[0_0_120px_-10px_rgba(121,194,76,0.22)]">
          ...
          <div className="mb-6 flex items-center justify-center gap-3">
            ...
            <a href={backHref} aria-label="Islam.wiki home" className="flex items-center gap-2">
              <img src="/icon.png" alt="Islam.wiki" width={48} height={48} className="rounded" loading="lazy" />
          ...
          {/* Subtitle */}
          <p className="mb-4 text-center text-sm text-iw-text-muted">
            Your account will sync all settings, preferences, and history across all of your devices.
          </p>

          {/* Settings link */}
          <div className="mb-5 text-center">
            <a href="/account/settings" className="inline-flex items-center gap-1.5 text-xs text-iw-text-muted transition-colors hover:text-iw-accent">
