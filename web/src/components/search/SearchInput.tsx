/**
 * SearchInput.tsx — Astro island port of components/search/search-input.tsx
 *
 * PURPOSE: Interactive search box with debounced live-preview dropdown. Renders on
 *   the /search page (and anywhere a quick-search box is needed) via client:load.
 * CONVERSION:
 *   - Removed 'use client' (Astro islands are plain React components).
 *   - next/navigation useRouter().push() → window.location.assign() (full nav; the
 *     destination is a server-rendered Astro page, so a hard nav is correct).
 *   - next/link <Link> → plain <a> (island has no client-side router).
 * SECURITY: Highlighting uses React text nodes (highlightMatch returns ReactNode with
 *   the matched substring inside <mark>), so user input is never injected as raw HTML —
 *   XSS-safe by construction (no dangerouslySetInnerHTML here). Results come from the
 *   server /api/search route which already sanitizes snippets.
 * REF: ports components/search/search-input.tsx (P2-E3 search migration)
 */
