/**
 * WikiGrid.tsx — Astro island port of components/listings/wiki-grid.tsx
 *
 * PURPOSE: Searchable, category-tabbed grid of wiki pages.
 * CONVERSION: next/link → <a>; copied verbatim minus 'use client'.
 * REF: ports components/listings/wiki-grid.tsx (articles-wiki migration group)
 */
...
export function WikiGrid({ pages }: { pages: WikiItem[] }) {
  ...
  return (
    <PaginatedGrid<WikiItem>
      ...
      tabs={tabs}
      tabFilterFn={(item, tabKey) => tabKey === 'all' || item.category === tabKey}
      filterFn={(item, query) => {
      ...
      renderItem={(page) => (
        <a href={`/wiki/${page.slug}`} className="card group">
