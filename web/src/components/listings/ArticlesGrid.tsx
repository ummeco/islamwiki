/**
 * ArticlesGrid.tsx — Astro island port of components/listings/articles-grid.tsx
 *
 * PURPOSE: Searchable, category-tabbed grid of encyclopedia articles.
 * CONVERSION: next/link → <a>; copied verbatim minus 'use client'.
 * REF: ports components/listings/articles-grid.tsx (articles-wiki migration group)
 */
...
export function ArticlesGrid({ articles }: { articles: ArticleItem[] }) {
  return (
    <PaginatedGrid<ArticleItem>
      ...
      renderItem={(article) => (
        <a href={`/articles/${article.slug}`} className="card group h-full">
          <span className="badge bg-iw-accent/10 text-iw-accent mb-2">
            {article.category}
          </span>
          ...
          {article.excerpt && (
            <p className="mt-2 line-clamp-3 text-sm text-iw-text-secondary">
              {article.excerpt}
            </p>
          )}
