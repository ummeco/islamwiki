/**
 * BooksGridIsland.tsx — Astro island for the books listing grid.
 *
 * PURPOSE: Client-side searchable/paginated/tabbed grid of classical books.
 *   Ports components/listings/books-grid.tsx to an Astro island: next/link →
 *   plain <a>, accepts a serializable items array (render fn cannot cross the
 *   island boundary), reuses the framework-independent PaginatedGrid + the
 *   shared formatIslamicYear helper verbatim.
 * INPUTS: books — BookItem[] (already filtered to canonical on the server page).
 * OUTPUTS: Interactive grid hydrated via client:load.
 * CONSTRAINTS: No next/* imports. No DOM-only globals at module scope.
 * REF: ports components/listings/books-grid.tsx · D-P2-STACK-CANON
 */
...
export default function BooksGridIsland({ books }: { books: BookItem[] }) {
  ...
  return (
    <PaginatedGrid<BookItem>
      ...
      renderItem={(book) => {
        ...
        return (
          <a href={`/books/${book.slug}`} className="card group h-full">
            ...
            <p className="mt-2 text-xs text-iw-text-secondary">
              By <span className="text-iw-accent">{book.author_name_en}</span>
