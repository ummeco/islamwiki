/**
 * SIEGE-03 H-05: Reading-plan REST API endpoint.
 *
 ...
 * progress via JWT (x-hasura-user-id claim).
 *
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  if (!planSlug) {
    return NextResponse.json(
      { error: 'plan query param is required.' },
      { status: 400 }
    )
  }
  ...
  if (!/^[a-z0-9-]{2,80}$/.test(planSlug)) {
    return NextResponse.json(
      { error: 'Invalid plan slug format.' },
      { status: 400 }
    )
  }
  ...
  const todaySections =
    planMeta.sections_per_day[currentDay - 1] ?? []
