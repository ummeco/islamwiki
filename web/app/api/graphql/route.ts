/**
 * S9-31: Islam.wiki — Hasura Remote Schema endpoint
 *
 ...
 *
 * Hasura authenticates every request with x-remote-schema-secret.
 * Data is served from static JSON files (no DB round-trip for these read-only
 * corpus queries — aligns with the static generation strategy).
 */

import { NextRequest, NextResponse } from 'next/server'
import { getSurahs, getSurahBySlug, getAyah } from '@/lib/data/quran'
...
const SDL = `
  ...
  type Query {
    _islamwiki: Boolean
    quranSurah(slug: String!): QuranSurah
    quranAyah(surahSlug: String!, number: Int!): QuranAyah
    hadithEntry(collection: String!, book: String!, number: Int!): HadithEntry
  }
`
...
export async function POST(req: NextRequest) {
  ...
  return NextResponse.json({ data: { [field]: result } })
}
