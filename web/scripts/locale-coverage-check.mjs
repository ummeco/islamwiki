#!/usr/bin/env node
/**
 * FILE:        islamwiki/web/scripts/locale-coverage-check.mjs
 ...
 *   - Warn-only locales excluded from this gate
...
const MESSAGES_DIR = join(__dirname, '../lib/i18n/messages');
...
if (!allPassed) {
  console.error('\nlocale-coverage-check FAILED — islamwiki-web');
  process.exit(1);
}
console.log('\nlocale-coverage-check PASSED — islamwiki-web (5 locales at 100%)');
process.exit(0);
